...
const linkClass =
  "text-claim underline decoration-claim/40 underline-offset-2 transition-colors hover:decoration-claim";
...
function renderText(node: LexicalNode, key: string): ReactNode {
  ...
  if (format & CODE) {
    element = (
      <code className="bg-chalk px-1.5 py-0.5 font-mono text-[0.9em] text-ink ring-1 ring-rule">
        {element}
      </code>
    );
  }
  ...
  switch (node.type) {
    ...
    case "paragraph": {
      ...
      return (
        <p
          key={key}
          className="my-5 leading-[1.75] text-ink"
    ...
    case "quote":
      return (
        <blockquote
          key={key}
          className="my-8 border-l-2 border-claim bg-chalk py-4 pl-5 pr-4 text-lg leading-relaxed text-ink"
        >
          {childrenOf(node, key)}
        </blockquote>
      );
    ...
    case "listitem":
      return (
        <li key={key} className="leading-[1.7] text-ink">
          {childrenOf(node, key)}
        </li>
      );
    ...
    case "horizontalrule":
      return <hr key={key} className="my-10 border-t-2 border-ink" />;
