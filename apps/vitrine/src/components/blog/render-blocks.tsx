...
function Figure({
  ...
  return (
    <figure className={`my-8 ${imageWidth[size ?? "normal"] ?? ""}`}>
      ...
      />
      {caption && (
        <figcaption className="mt-2 font-mono text-[11px] uppercase tracking-[0.14em] text-slate">
          {caption}
        </figcaption>
      )}
    </figure>
        ...
        switch (block.blockType) {
          ...
          case "quote":
            return (
              <figure key={key} className="my-10">
                <blockquote className="border-l-2 border-claim bg-chalk py-5 pl-6 pr-5 text-xl leading-relaxed text-ink">
                  {block.quote}
                </blockquote>
                {block.attribution && (
                  <figcaption className="mt-3 pl-6 font-mono text-[11px] uppercase tracking-[0.16em] text-slate">
                    — {block.attribution}
                  </figcaption>
                )}
              </figure>
          ...
          case "cta": {
            ...
            return (
              <aside
                key={key}
                className="my-12 border-2 border-ink bg-chalk p-6 offset-ink md:p-8"
              >
          ...
          case "code":
            return (
              <div key={key} className="my-8 border-2 border-ink bg-ink">
                {block.language && (
                  <p className="border-b border-slate/40 px-4 py-2 font-mono text-[10px] uppercase tracking-[0.18em] text-vacant">
                    {block.language}
                  </p>
                )}
                <pre className="overflow-x-auto p-4">
                  <code className="font-mono text-[13px] leading-relaxed text-paper">
                    {block.code}
                  </code>
                </pre>
              </div>
