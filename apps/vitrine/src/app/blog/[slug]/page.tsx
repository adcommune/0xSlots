  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-6xl px-4 py-16 md:px-6 md:py-24">
        <Link
          href="/blog"
          className="font-mono text-[11px] uppercase tracking-[0.16em] text-slate transition-colors hover:text-claim"
        >
          ← All writing
        </Link>
        ...
        <article className="mt-10">
          <header className="border-b-2 border-ink pb-8">
            {hero && (
              <div className="mb-8 inline-flex border-2 border-ink bg-chalk p-3">
                <Image
                  src={hero}
                  alt={post.title}
                  width={64}
            ...
            <div className="mt-6 flex flex-wrap items-center gap-x-4 gap-y-2 font-mono text-[11px] uppercase tracking-[0.16em] text-slate">
              ...
              {categories.map((category) => (
                <span
                  key={category.id}
                  className="border border-rule px-2 py-0.5"
                >
                  {category.title}
                </span>
              ))}
